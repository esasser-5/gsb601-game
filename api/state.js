// Vercel KV API route — GET/POST game state and connections
// GET  /api/state?session=gsb601-2025              → returns { revealed, playerCount, connections }
// POST /api/state  body: { action, ...params }     → mutate state
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'join') {
      ...
      // Auto-expire player set entries after 2 hours
    ...
    if (action === 'connect') {
      // Store as "playerId:canId" → clueId
      await kv.hset(`${SESSION}:connections`, {
        [`${playerId}:${canId}`]: clueId
      });
